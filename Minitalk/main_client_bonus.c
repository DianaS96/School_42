...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/01/11 14:06:39 by coverand          #+#    #+#             */
/*   Updated: 2022/01/12 16:02:24 by coverand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minitalk.h"

static void	ft_server_response(int sig);
...
static void	send_message(int pid, char *str)
{
	...
	while (i <= len)
	{
		...
		while (counter >= 1)
		{
			...
			counter /= 2;
			usleep(600);
			signal(SIGUSR2, ft_server_response);
...
{
	if (sig == SIGUSR1)
		ft_putstr(FGREEN"Message was received by server.\n"NONE);
}
